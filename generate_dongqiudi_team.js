...
if (!TEAM_ID) {
    throw new Error('DONGQIUDI_TEAM_ID must not be empty');
}
...
async function getTeamMeta() {
    try {
        ...
        return {
            name: cleanTeamName(rawName) || TEAM_NAME || `懂球帝球队 ${TEAM_ID}`,
            image,
        };
    } catch (error) {
        console.warn(`Could not read team metadata: ${error.message}`);
        return {
            name: TEAM_NAME || `懂球帝球队 ${TEAM_ID}`,
            image: '',
        };
    }
}
...
function getArticleList(payload) {
    ...
    const articles = candidates.find(Array.isArray);
    if (!articles) {
        throw new Error('Unexpected Dongqiudi API payload: articles array not found');
    }
...
function getArticleSummary(article) {
    const candidates = [article?.summary, article?.description, article?.digest, article?.intro, article?.subtitle];
...
async function generateRss() {
    try {
        ...
        if (!articles.length) {
            throw new Error('Dongqiudi API returned no usable articles');
        }
