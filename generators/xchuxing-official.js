...
const OUTPUT_PATH = process.env.OUTPUT_PATH || path.join(__dirname, '..', 'feeds', 'xchuxing', 'official.xml');
...
function extractDescription(article) {
    ...
    if (type === 7 && article && Array.isArray(article.options)) {
        ...
        baseDesc = baseDesc ? `${baseDesc}<br><br>${optionsList}` : optionsList;
    }
...
async function generateRSS() {
    try {
        ...
        if (!scriptContent) {
            throw new Error('Could not find Nuxt data script');
        }
        ...
        const unflatten = (index, cache = new Map()) => {
            ...
            if (typeof val === 'object') {
                ...
                const obj = {};
                cache.set(index, obj);
                for (const k in val) obj[k] = unflatten(val[k], cache);
                return obj;
            }
            return val;
        };
        ...
        for (let i = 0; i < rawData.length; i++) {
            ...
            if (item && typeof item === 'object' && !Array.isArray(item) && Object.hasOwn(item, 'list') && Object.hasOwn(item, 'category')) {
                listContainerIdx = i;
                break;
            }
        }
        ...
        if (listContainerIdx === -1) {
            for (let i = 0; i < rawData.length; i++) {
                const item = rawData[i];
                if (item && typeof item === 'object' && !Array.isArray(item) && Object.hasOwn(item, 'list')) {
                    listContainerIdx = i;
                    break;
                }
            }
        ...
        if (listContainerIdx === -1) throw new Error('Failed to locate content');

        const contentData = unflatten(listContainerIdx);
        if (!contentData || !Array.isArray(contentData.list)) throw new Error('Content data structure is invalid');

        const filtered = [...contentData.list]
            .filter(Boolean)
            .sort((a, b) => Number(b.created_at || 0) - Number(a.created_at || 0))
            .slice(0, Number.isFinite(MAX_ITEMS) ? Math.max(1, MAX_ITEMS) : 50)
            .map((article) => ({ article, url: buildItemUrl(article) }))
            .filter(({ article, url }) => shouldIncludeArticle(article, url));
        ...
        for (const { article, url } of filtered) {
            ...
            if (ONE_IMAGE_TITLE_PREFIX_RE.test(title)) {
                try {
                    ...
                    // A failed optional image lookup must not invalidate the feed.
                }
            }
